//
//  ZTViewSelectorController.m
//  ViewSelector
//
//  Created by ZT on 16/8/8.
//  Copyright © 2016年 leehyoley. All rights reserved.
//

#import "ZTViewSelectorController.h"
#define ZTScreenWidth [[UIScreen mainScreen] bounds].size.width
@interface ZTViewSelectorController ()
...
@implementation ZTViewSelectorController

-(instancetype)initWithControllers:(NSArray<UIViewController*>*)controllers titles:(NSArray<NSString*>*)titles frame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {

        self.controllers = controllers;
        self.titles = titles;
        //默认
        self.fontSize = autoScaleW(15);
        self.tipSize = CGSizeMake(40, 2);
        self.normalColor = [UIColor blackColor];
        self.selectedColor = RGB(236, 125, 123);
        [self initWithView];
    }
    return self;
}
...
- (void)initWithView {
    ...
    self.selectorView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ZTScreenWidth,autoScaleH(45))];
    self.showView = [[UIView alloc] initWithFrame:CGRectMake(0, self.selectorView.frame.size.height, ZTScreenWidth, self.bounds.size.height - self.selectorView.frame.size.height)];
    
    NSMutableArray<UIButton*> *buttons = [NSMutableArray array];
    for (int i=0; i<self.controllers.count; i++) {
        UIViewController *vc = self.controllers[i];
        vc.view.frame = CGRectMake(i*self.showView.bounds.size.width, 0, self.showView.bounds.size.width, self.showView.bounds.size.height - autoScaleH(50));
        ...
        if (i==0) {
            ...
            linelabel.sd_layout.rightEqualToView(button).topEqualToView(button).widthIs(1).heightIs(button.frame.size.height);
        }
    }
    self.titleButtons = [NSArray arrayWithArray:buttons];
    self.tipView = [[UIView alloc] initWithFrame:CGRectMake(0, self.selectorView.bounds.size.height-2, 60, 3)];
    self.tipView.center = CGPointMake(self.titleButtons[0].center.x, self.tipView.center.y);
    self.tipView.backgroundColor = RGB(236, 125, 123);
...
-(void)didClickItem:(UIButton*)sender{
    ...
    if (sender.tag == 0) {
        self.showView.frame = CGRectMake(0, self.selectorView.height_sd, ZTScreenWidth, self.bounds.size.height - self.selectorView.height_sd-autoScaleH(50));
                
    } else {
        self.showView.frame = CGRectMake(0, self.selectorView.frame.size.height, ZTScreenWidth, autoScaleH(603 - 50));
    }
    [UIView animateWithDuration:0.3 animations:^{
