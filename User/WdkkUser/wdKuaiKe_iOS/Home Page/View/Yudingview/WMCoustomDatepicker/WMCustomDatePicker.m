...
- (id)initWithframe:(CGRect)frame Delegate:(id<WMCustomDatePickerDelegate>)delegate PickerStyle:(WMDateStyle)WMDateStyle Day:(NSInteger)dayint
{
...
//    [self DaysfromYear:[model.year integerValue] andMonth:[model.month integerValue]];
    ...
    if ([model.minute intValue]>=55-MINUTEADD) {
        hourIndex += 1;
        hourIndex1 += 1;
    }
...
- (NSInteger)findMinuteIndex:(NSString *)minute
{
    for ( int i = 0; i<60; i++) {
        ...
        if ([minute integerValue]%DATEPICKER_interval==0) {
            ...
            if([minute integerValue]/DATEPICKER_interval==12){
...
//    NSInteger dis = 7; //前后的天数
    ...
    for (int i =0; i<7; i++) {
        ...
        NSString * dayStr = [NSString stringWithFormat:@"%@日",daystr];
