...
+ (instancetype)indexPathWithCol:(NSInteger)col leftOrRight:(NSInteger)leftOrRight leftRow:(NSInteger)leftRow row:(NSInteger)row {
    ZTIndexPath *indexPath = [[self alloc] initWithColumn:col leftOrRight:leftOrRight leftRow:leftRow row:row];
    return indexPath;
}
@end
...
@property (nonatomic, assign) CGFloat offset_tabV;
...
#pragma mark - setter
- (void)setDataSource:(id<ZTDropDownMenuDataSource>)dataSource {
...
- (void)contentOffsetAction:(NSNotification *)notification{
   CGFloat offset =  [notification.userInfo[@"contentoffset"] integerValue];
//    NSLog(@" 监听偏移量 －－ 传给自定义菜单类 %f ", offset);
    _offset_tabV = offset;
}
#pragma mark - init method
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {

    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(contentOffsetAction:) name:@"contentoffset" object:nil];
    ...
    if (self) {
        ...
        [_collectionView registerClass:[ZTCollectionViewCell class] forCellWithReuseIdentifier:@"CollectionCell"];
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, 650, screenSize.width, screenSize.height)];
...
#warning 设置点击后滑动到顶端  根据计算首页布局，默认为560，若改变布局需重新设置此数值。
    UITableView *homeTabV = [self superview];

    homeTabV.scrollEnabled = NO;
    CGPoint touchPoint = [paramSender locationInView:self];
    NSLog(@"------%f", _offset_tabV);
    //calculate index
...
//        UITableView *leftTableView = _leftTableView;
    ...
    homeTabV.contentOffset = CGPointMake(0, 550 + (_offset_tabV == 0 ? 0 : (_offset_tabV - 550)));
}

- (void)backgroundTapped:(UITapGestureRecognizer *)paramSender
{    UITableView *homeTabV = [self superview];
    homeTabV.scrollEnabled = YES;
    BOOL displayByCollectionView = NO;
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        ...
        titleLabel.text = [self.dataSource menu:self titleForRowAtIndexPath:[ZTIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:indexPath.row]];
        // 只取宽度
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
        ...
        [self.delegate menu:self didSelectRowAtIndexPath:[ZTIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:indexPath.row]];
...
- (void)confiMenuWithSelectRow:(NSInteger)row leftOrRight:(NSInteger)leftOrRight{
    ...
    title.string = [self.dataSource menu:self titleForRowAtIndexPath:[ZTIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:row]];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZTCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:collectionCell forIndexPath:indexPath];
    
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        cell.textLabel.text = [self.dataSource menu:self titleForRowAtIndexPath:[ZTIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 row:indexPath.row]];
    } else {
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
        ...
        [self.delegate menu:self didSelectRowAtIndexPath:[ZTIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 row:indexPath.row]];
    } else {
        //TODO: delegate is nil
    }
}
...
- (void)confiMenuWithSelectRow:(NSInteger)row{
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    title.string = [self.dataSource menu:self titleForRowAtIndexPath:[ZTIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 row:row]];
