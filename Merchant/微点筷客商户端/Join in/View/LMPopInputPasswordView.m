...
@property(nonatomic,strong)ButtonStyle *cancelButton;//取消按钮
@property(nonatomic,strong)ButtonStyle *ensureButton;//确定按钮
@end
...
-(void)customInit
{
    ...
    _cancelButton = [ButtonStyle buttonWithType:UIButtonTypeCustom];
    ...
    _ensureButton = [ButtonStyle buttonWithType:UIButtonTypeCustom];
...
//    _lineLabel.frame = CGRectMake(offsetX, offsetY, frame.size.width - 2 * offsetX, 1);
    ...
    offsetY += _titleLabel.frame.size.height + 17;
...
-(void)buttonClickedAction:(ButtonStyle *)sender
{
    //传值给委托对象
