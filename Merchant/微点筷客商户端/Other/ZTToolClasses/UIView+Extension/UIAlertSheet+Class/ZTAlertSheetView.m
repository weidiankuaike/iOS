...
- (void)create:(NSArray *)array{
    ...
    maskView = [[UIView alloc] initWithFrame:ZTMainScreen];
    ...
    CGFloat start_x = (8);
    CGFloat width = (kScreenWidth - start_x * 2);
    CGFloat height = (60);
    CGFloat start_y = (kScreenHeight - height - start_x);

    ButtonStyle *bottomBT = [ButtonStyle buttonWithType:UIButtonTypeCustom];
...
- (void)showView{
    ...
    maskView.alpha = 0;
    self.frame = CGRectMake(0, topBackView.origin.y, kScreenWidth, kScreenHeight);
...
- (void)dismiss{
    ...
    [UIView animateWithDuration:.35 animations:^{
        self.frame = CGRectMake(0, topBackView.frame.origin.y, kScreenWidth, kScreenHeight);
        maskView.alpha = 0.0;
    } completion:^(BOOL finished) {
        if (finished) {
...
- (void)buttonAction:(ButtonStyle *)sender{
    [self dismiss];
    self.alertSheetReturn(sender.tag - 40000);
}
- (void)bottomAction:(ButtonStyle *)sender{
    [self dismiss];
    self.alertSheetReturn(arrCount - 1);
}
@end
