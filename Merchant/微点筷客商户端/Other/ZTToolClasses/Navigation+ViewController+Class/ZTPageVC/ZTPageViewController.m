...
@interface ZTPageViewController ()<UIPageViewControllerDataSource,UIPageViewControllerDelegate,UIScrollViewDelegate,ZTSegmentViewDelegate>
{
    BOOL _done; //是否翻页完成
...
@property (nonatomic,strong)ZTSegmentView *viewSegment;

@end

@implementation ZTPageViewController
...
#pragma mark - ZTSegmentViewDelegate
- (void)ZTSegmentView:(ZTSegmentView *)segmentView didClickTagAtIndex:(NSInteger)index{
    
    if (index == _currentPage) {
        return;
    }
    
    __weak typeof(ZTPageViewController *)weakSelf = self;
...
//头部标签视图
- (ZTSegmentView *)viewSegment{
    
    if (!_viewSegment) {
        _viewSegment = [[ZTSegmentView alloc] initWithTitles:self.titles];
