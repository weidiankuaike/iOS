...
#define kSEType         @"type"
#define kSearchServiceCBUUIDArr  @[[CBUUID UUIDWithString:@"E7810A71-73AE-499D-8C15-FAA9AEF0C3F2"]]
#define kSearchCharacterOptionDic  @{CBConnectPeripheralOptionNotifyOnDisconnectionKey:@(YES)}

// 发送数据时，需要分段的长度，部分打印机一次发送数据过长就会乱码，需要分段发送。这个长度值不同的打印机可能不一样，你需要调试设置一个合适的值（最好是偶数）
#define kLimitLength    82//146
...
- (void)startScanPerpheralTimeout:(NSTimeInterval)timeout
{
    ...
    if (_centralManager.state == CBManagerStatePoweredOn) {
        [_centralManager scanForPeripheralsWithServices:kSearchServiceCBUUIDArr options:kSearchCharacterOptionDic];
        return;
    }
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central
{
    if (central.state != CBManagerStatePoweredOn) {
        ...
        [central scanForPeripheralsWithServices:kSearchServiceCBUUIDArr options:kSearchCharacterOptionDic];
    }
}
