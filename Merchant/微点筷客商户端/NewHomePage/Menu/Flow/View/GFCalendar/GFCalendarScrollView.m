...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.didSelectDayHandler != nil) {
        ...
        if (_typeint==1) {//区间
            ...
            if (a==3) {
                [MBProgressHUD showError:@"已重新选择区间"];
                a = 1;
                for (GFCalendarCell * gcell in _collectionAry) {
                    
                    gcell.todayCircle.backgroundColor = [UIColor clearColor];
                    gcell.todayLabel.textColor = [UIColor blackColor];
                }
                [_collectionAry removeAllObjects];
            }
        }else if (_typeint==2){//单选
        ...
        cell.todayCircle.backgroundColor = [UIColor greenColor];
        cell.todayLabel.textColor = [UIColor whiteColor];
        [_collectionAry addObject:cell];
        ...
        if (currentMonthInfo.month ==[[NSDate date] dateMonth]) {

           if (day<=[[NSDate date] dateDay] ) {
               self.didSelectDayHandler(year, month, day,a);

            }
            else
            {
                a -=1;
                [MBProgressHUD showError:@"当前选择日期不符合！"];
        ...
        else if (currentMonthInfo.month <[[NSDate date]dateMonth]){
//            a +=1;
//            
//            if (a==3) {
//                
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    // 向右滑动
    float x = ceilf(scrollView.contentOffset.x);
    NSString * str = [NSString stringWithFormat:@"%.0lf", x];
    
    NSInteger xx = [str integerValue];
    
    float wind = ceilf(self.frame.size.width);
    NSString * windstr = [NSString stringWithFormat:@"%.01f",wind];
    NSInteger windxx = [windstr integerValue];
