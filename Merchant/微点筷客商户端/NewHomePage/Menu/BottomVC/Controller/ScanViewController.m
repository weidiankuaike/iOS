            ...
            if ([result[@"msgType"] integerValue] == 0) {
                ...
                if ([result[@"obj"][@"isBind"] integerValue] == 0) {
                    ...
                    if (self.returnScanResult) {
                        self.returnScanResult(codeInfo);
...
//                        ZTLog(@"__+++");
                    ...
                    //不为空，该id已经绑定桌号，弹窗提示
//                    ZTLog(@"++");
                    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"已绑定" message:@"该二维码已绑定桌号" preferredStyle:UIAlertControllerStyleAlert];
